...
- **i18n**: all user-visible strings go through `{{ trans('key') }}` in Twig or `Lang::get('key')` in PHP. Add new keys to `lang/en.php` first — other locale files fall back to English automatically via the 3-layer chain: `en.php` → `{base}.php` → `{locale}.php`. That fallback exists for robustness, not as a substitute for translating: whenever a key is added, removed, or its English text changes, update every other locale file in `lang/` in the same change (translating new/changed keys, matching each file's existing tone) — don't leave the fallback to carry it long-term. `fr-CA.php` and `pt-PT.php` are the sole exception: they're intentionally partial regional-override files (deltas on top of `fr.php`/`pt.php`), not full translations, so they only need updates for strings that differ from their base language.
