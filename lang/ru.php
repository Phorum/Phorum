<?php
declare(strict_types=1);

/**
 * Russian translations for Phorum.
 * Machine-translated — please review with a native speaker.
 */
return [
    '_name' => 'Русский',

    // Navigation
    'nav.forum_index'   => 'Список форумов',
    'nav.search'        => 'Поиск',
    'nav.messages'      => 'Сообщения',
    'nav.groups'        => 'Группы',
    'nav.settings'      => 'Настройки',
    'nav.log_out'       => 'Выйти',
    'nav.log_in'        => 'Войти',
    'nav.register'      => 'Регистрация',
    'nav.powered_by'    => 'Работает на Phorum',
    'nav.skip_to_content' => 'Перейти к основному содержимому',
    'nav.breadcrumb'    => 'Хлебные крошки',
    'nav.primary'       => 'Основное',
    'nav.menu'          => 'Меню',
    'pagination.nav_label' => 'Навигация по страницам',

    // Forum list
    'forum_list.no_forums'   => 'Форумы ещё не созданы.',
    'forum_list.col_forum'   => 'Форум',
    'forum_list.col_posts'   => 'Сообщений',
    'forum_list.col_threads' => 'Тем',
    'forum_list.col_last'    => 'Последнее сообщение',

    // Forum view
    'forum.new_thread'       => 'Новая тема',
    'forum.no_threads'       => 'Тем пока нет.',
    'forum.start_one'        => 'Создайте первую.',
    'forum.col_subject'      => 'Тема',
    'forum.col_author'       => 'Автор',
    'forum.col_replies'      => 'Ответов',
    'forum.col_posts'        => 'Сообщений',
    'forum.col_last_post'    => 'Последнее сообщение',
    'forum.sticky'           => 'Закреплено',
    'forum.closed'           => 'Закрыто',
    'forum.by'               => 'от',
    'forum.new'              => 'новое',
    'forum.mark_read'        => 'Отметить всё прочитанным',
    'forum.feed_link'        => 'RSS',

    // Thread actions
    'thread.reply'           => 'Ответить',
    'thread.follow'          => 'Подписаться',
    'thread.following'       => 'Вы подписаны',
    'thread.reopen'          => 'Открыть тему',
    'thread.close'           => 'Закрыть тему',
    'thread.sticky'          => 'Закрепить',
    'thread.unsticky'        => 'Открепить',
    'thread.move'            => 'Переместить',
    'thread.merge'           => 'Объединить',
    'thread.delete'          => 'Удалить тему',

    // Message actions
    'message.awaiting_approval' => 'Ожидает проверки',
    'message.reply'             => 'Ответить',
    'message.edit'              => 'Изменить',
    'message.edit_title'        => 'Редактировать сообщение',
    'message.save_edit'         => 'Сохранить изменения',
    'message.edited_note'       => 'Изменено',
    'message.changes'           => 'Changes',
    'message.changes_title'     => 'Edit History',
    'message.changes_edit_n'    => 'Edit #{n}',
    'message.changes_by'        => 'by',
    'message.changes_subject'   => 'Subject',
    'message.changes_body'      => 'Message',
    'message.changes_back'      => 'Back to thread',
    'message.approve'           => 'Одобрить',
    'message.delete'            => 'Удалить',
    'message.report'            => 'Пожаловаться',
    'message.registered'        => 'Зарегистрирован',
    'message.posts'             => 'Сообщений',
    'message.ip_address'        => 'IP-адрес',

    // Post form
    'post.new_thread'        => 'Новая тема',
    'post.reply_to'          => 'Ответ на: {subject}',
    'post.reply'             => 'Ответить',
    'post.subject'           => 'Тема',
    'post.body'              => 'Сообщение',
    'post.submit_thread'     => 'Создать тему',
    'post.submit_reply'      => 'Отправить ответ',
    'post.cancel'            => 'Отмена',
    'post.preview'           => 'Предпросмотр',
    'post.error_subject_required' => 'Необходимо указать тему.',
    'post.error_subject_length'   => 'Тема должна содержать не более 255 символов.',
    'post.error_body_required'    => 'Необходимо ввести текст сообщения.',
    'post.error_flood_wait'       => 'Пожалуйста, подождите ещё {seconds} секунд(ы), прежде чем публиковать снова.',
    'post.error_posting_blocked'  => 'Публикация сообщений для вашего аккаунта запрещена.',
    'post.error_duplicate'       => 'Вы уже недавно публиковали это сообщение.',

    // Authentication
    'auth.login_title'       => 'Вход',
    'auth.logout_title'       => 'Выйти',
    'auth.logout_confirm'       => 'Вы действительно хотите выйти?',
    'auth.logout_submit'       => 'Выйти',
    'auth.logout_already'       => 'Вы не вошли в систему.',
    'auth.username'          => 'Имя пользователя',
    'auth.password'          => 'Пароль',
    'auth.remember_me'       => 'Запомнить меня',
    'auth.login_submit'      => 'Войти',
    'auth.create_account'    => 'Создать аккаунт',
    'auth.register_title'    => 'Создание аккаунта',
    'auth.email'             => 'Адрес электронной почты',
    'auth.password_hint'     => 'не менее 6 символов',
    'auth.confirm_password'  => 'Подтверждение пароля',
    'auth.register_submit'   => 'Создать аккаунт',
    'auth.have_account'      => 'Уже есть аккаунт?',
    'auth.login_link'        => 'Войдите.',
    'auth.forgot_password'   => 'Забыли пароль?',
    'auth.forgot_title'      => 'Сброс пароля',
    'auth.forgot_email_label' => 'Адрес электронной почты',
    'auth.forgot_submit'     => 'Отправить ссылку для сброса',
    'auth.forgot_sent'       => 'Если этот адрес электронной почты зарегистрирован, на него отправлена ссылка для сброса пароля. Проверьте входящие.',
    'auth.reset_title'       => 'Придумайте новый пароль',
    'auth.reset_new_password' => 'Новый пароль',
    'auth.reset_confirm'     => 'Подтверждение нового пароля',
    'auth.reset_submit'      => 'Установить новый пароль',
    'auth.reset_invalid'     => 'Ссылка для сброса пароля недействительна или устарела. Запросите новую.',
    'auth.reset_success'     => 'Ваш пароль обновлён. Вы вошли в систему.',
    'auth.confirm_pending_title'  => 'Проверьте вашу почту',
    'auth.confirm_pending_body'   => 'Мы отправили ссылку для подтверждения на {email}. Перейдите по ней, чтобы активировать аккаунт.',
    'auth.confirm_pending_resend' => 'Отправить письмо с подтверждением повторно',
    'auth.confirm_invalid'        => 'Эта ссылка для подтверждения недействительна или устарела.',
    'auth.confirm_success_login'        => 'Ваш адрес электронной почты подтверждён. Пожалуйста, войдите.',
    'auth.resend_title'           => 'Повторная отправка письма с подтверждением',
    'auth.resend_email_label'     => 'Адрес электронной почты',
    'auth.resend_submit'          => 'Отправить повторно',
    'auth.resend_sent'            => 'Если для этого адреса есть ожидающее подтверждение, новая ссылка отправлена. Проверьте входящие.',
    'auth.pending_approval_title' => 'Ожидание одобрения модератора',
    'auth.pending_approval_body'  => 'Спасибо! Ваша учётная запись теперь ожидает проверки и одобрения модератором. Вы сможете войти, как только это произойдёт.',
    'auth.error_missing_credentials'  => 'Пожалуйста, введите имя пользователя и пароль.',
    'auth.error_invalid_credentials'  => 'Неверное имя пользователя или пароль.',
    'auth.error_throttled'  => 'Слишком много попыток. Подождите {seconds} секунд(ы) и попробуйте снова.',
    'auth.error_registration_blocked' => 'Регистрация для вашего аккаунта запрещена.',
    'auth.error_invalid_email'        => 'Пожалуйста, введите корректный адрес электронной почты.',
    'auth.error_password_min_length'  => 'Пароль должен содержать не менее 6 символов.',
    'auth.error_passwords_mismatch'   => 'Пароли не совпадают.',
    'auth.error_username_required'    => 'Необходимо указать имя пользователя.',
    'auth.error_username_length'      => 'Имя пользователя должно содержать от 2 до 50 символов.',
    'auth.error_email_required'       => 'Необходимо указать корректный адрес электронной почты.',
    'auth.error_username_taken'       => 'Это имя пользователя уже занято.',
    'auth.error_email_taken'       => 'Этот адрес электронной почты уже зарегистрирован. Попробуйте войти или воспользуйтесь «Забыли пароль».',

    // OAuth login
    'oauth.button_google' => 'Продолжить через Google',
    'oauth.button_github' => 'Продолжить через GitHub',
    'oauth.error_provider_error'        => 'Вход был отменён, или провайдер вернул ошибку. Пожалуйста, попробуйте снова.',
    'oauth.error_state_mismatch'        => 'Ваша сессия входа истекла или недействительна. Пожалуйста, попробуйте снова.',
    'oauth.error_token_exchange_failed' => 'Не удалось завершить вход через этого провайдера. Пожалуйста, попробуйте снова.',
    'oauth.error_email_not_verified'    => 'Ваш адрес электронной почты не подтверждён у этого провайдера, поэтому мы не можем выполнить вход. Пожалуйста, подтвердите почту у провайдера и попробуйте снова.',
    'oauth.error_login_failed'          => 'Что-то пошло не так при входе. Пожалуйста, попробуйте снова.',
    'oauth.error_account_inactive'      => 'Ваша учётная запись ещё не активна. Проверьте почту для получения ссылки подтверждения.',
    'oauth.error_account_exists'      => 'На этом сайте уже есть учётная запись с этим адресом электронной почты. Войдите с помощью пароля или сначала подтвердите адрес через «Забыли пароль» — после этого можно будет использовать этого поставщика.',
    'oauth.error_not_configured'        => 'Этот способ входа сейчас недоступен.',

    // User profile
    'profile.username'       => 'Имя пользователя',
    'profile.name'           => 'Имя',
    'profile.email'          => 'Электронная почта',
    'profile.joined'         => 'Зарегистрирован',
    'profile.posts'          => 'Сообщений',
    'profile.last_active'    => 'Последняя активность',
    'profile.signature'      => 'Подпись',
    'profile.recent_posts'   => 'Последние сообщения',
    'profile.col_subject'    => 'Тема',
    'profile.col_date'       => 'Дата',
    'profile.edit_settings'  => 'Изменить настройки',
    'profile.continue_browsing' => 'Продолжить просмотр: {forum}',

    // Account settings
    'settings.title'             => 'Настройки аккаунта',
    'settings.saved'             => 'Настройки сохранены.',
    'settings.identity'          => 'Личные данные',
    'settings.display_name'      => 'Отображаемое имя',
    'settings.email'             => 'Адрес электронной почты',
    'settings.hide_email'        => 'Скрыть мой адрес электронной почты в профиле',
    'settings.password_section'  => 'Пароль',
    'settings.password_hint'     => 'Оставьте пустым, чтобы сохранить текущий пароль.',
    'settings.current_password'      => 'Текущий пароль',
    'settings.current_password_hint'      => 'Требуется только при смене пароля или адреса электронной почты.',
    'settings.new_password'      => 'Новый пароль',
    'settings.confirm_password'  => 'Подтверждение нового пароля',
    'settings.signature_section' => 'Подпись',
    'settings.signature_text'    => 'Текст подписи',
    'settings.show_signature'    => 'Показывать подпись в моих сообщениях',
    'settings.preferences'       => 'Предпочтения',
    'settings.threaded_read'     => 'Использовать древовидный вид при чтении тем',
    'settings.email_notify'          => 'Когда я создаю новую тему или отвечаю',
    'settings.email_notify_none'     => 'Не подписываться автоматически',
    'settings.email_notify_bookmark' => 'Подписываться без уведомлений (без писем)',
    'settings.email_notify_message'  => 'Подписываться и уведомлять меня по почте об ответах',
    'settings.pm_email_notify'   => 'Уведомлять меня по электронной почте при получении личного сообщения',
    'settings.tz_offset'         => 'Временная зона (часы, от −12 до +14; −99 = серверное время)',
    'settings.language'          => 'Язык',
    'settings.theme'             => 'Тема',
    'settings.save'              => 'Сохранить настройки',
    'settings.cancel'            => 'Отмена',
    'settings.avatar_section'    => 'Аватар',
    'settings.avatar_current'    => 'Текущий аватар',
    'settings.avatar_upload'     => 'Загрузить новый аватар',
    'settings.avatar_hint'       => 'JPG, PNG, GIF или WebP. Максимум 100 КБ.',
    'settings.avatar_delete'     => 'Удалить текущий аватар',
    'settings.error_display_name_required' => 'Необходимо указать отображаемое имя.',
    'settings.error_display_name_length'   => 'Отображаемое имя должно содержать не более 50 символов.',
    'settings.error_email_required'        => 'Необходимо указать корректный адрес электронной почты.',
    'settings.error_email_taken'           => 'Этот адрес электронной почты уже используется другим аккаунтом.',
    'settings.error_password_min_length'   => 'Новый пароль должен содержать не менее 6 символов.',
    'settings.error_current_password'   => 'Введите текущий пароль, чтобы изменить пароль или адрес электронной почты. Если вы регистрировались через Google или GitHub и никогда не задавали пароль, сначала задайте его по ссылке «Забыли пароль».',
    'settings.error_passwords_mismatch'    => 'Пароли не совпадают.',
    'settings.error_tz_offset'             => 'Смещение временной зоны должно быть от −12 до +14, либо −99 для серверного времени.',
    'settings.error_email_notify'          => 'Пожалуйста, выберите допустимый вариант подписки.',

    // Forced password change
    'force_password_change.title'     => 'Смена пароля',
    'force_password_change.message'   => 'Администратор требует установить новый пароль, прежде чем продолжить.',
    'force_password_change.new_password'     => 'Новый пароль',
    'force_password_change.confirm_password' => 'Подтверждение нового пароля',
    'force_password_change.save'      => 'Установить пароль',
    'force_password_change.error_password_min_length' => 'Новый пароль должен содержать не менее 6 символов.',
    'force_password_change.error_passwords_mismatch'  => 'Пароли не совпадают.',

    // Private messages
    'pm.private_messages'    => 'Личные сообщения',
    'pm.folders'             => 'Папки',
    'pm.inbox'               => 'Входящие',
    'pm.outbox'              => 'Отправленные',
    'pm.compose'             => 'Написать',
    'pm.manage_folders'      => 'Управление папками',
    'pm.no_messages'         => 'Нет сообщений.',
    'pm.col_subject'         => 'Тема',
    'pm.col_from'            => 'От',
    'pm.col_to'              => 'Кому',
    'pm.col_date'            => 'Дата',
    'pm.delete'              => 'Удалить',
    'pm.compose_title'       => 'Написать сообщение',
    'pm.to_label'            => 'Кому (имя пользователя)',
    'pm.subject'             => 'Тема',
    'pm.body'                => 'Сообщение',
    'pm.send'                => 'Отправить',
    'pm.cancel'              => 'Отмена',
    'pm.reply'               => 'Ответить',
    'pm.back_to_inbox'       => 'Назад во входящие',
    'pm.move_to_folder'      => 'Переместить в папку…',
    'pm.move'                => 'Переместить',
    'pm.delete_title'        => 'Удаление личного сообщения',
    'pm.delete_confirm'      => 'Удалить «{subject}» от {author}?',
    'pm.create_folder_title' => 'Создать папку',
    'pm.folder_name'         => 'Название папки',
    'pm.create'              => 'Создать',
    'pm.buddy_list'          => 'Buddy List',
    'pm.no_buddies'          => 'You have no buddies yet.',
    'pm.add_buddy'           => 'Add Buddy',
    'pm.remove_buddy'        => 'Remove Buddy',
    'pm.mutual'              => 'Mutual',
    'pm.col_buddy'           => 'User',
    'pm.col_mutual'          => 'Mutual',
    'pm.col_last_active'     => 'Last Active',
    'pm.error_recipient_required'   => 'Необходимо указать получателя.',
    'pm.error_user_not_found'       => 'Пользователь «{username}» не найден.',
    'pm.error_subject_required'     => 'Необходимо указать тему.',
    'pm.error_body_required'        => 'Необходимо ввести текст сообщения.',
    'pm.error_folder_name_required' => 'Необходимо указать название папки.',
    'pm.error_folder_name_length'   => 'Название папки должно содержать не более 60 символов.',

    // Groups (self-service join/leave + group moderator panel)
    'groups.title'             => 'Группы',
    'groups.my_groups'         => 'Мои группы',
    'groups.no_memberships'    => 'Вы не состоите ни в одной группе.',
    'groups.col_group'         => 'Группа',
    'groups.col_status'        => 'Статус',
    'groups.status_moderator'  => 'Модератор',
    'groups.status_approved'   => 'Участник',
    'groups.status_unapproved' => 'Ожидает одобрения',
    'groups.status_suspended'  => 'Заблокирован',
    'groups.manage'            => 'Управление',
    'groups.leave'             => 'Покинуть',
    'groups.joinable_groups'   => 'Группы, к которым можно присоединиться',
    'groups.no_joinable'       => 'Сейчас нет доступных групп для присоединения.',
    'groups.join'              => 'Подать заявку',
    'groups.moderate_title'    => 'Управление группой: {name}',
    'groups.pending_requests'  => 'Ожидающие заявки',
    'groups.no_pending'        => 'Нет ожидающих заявок.',
    'groups.col_user'          => 'Пользователь',
    'groups.approve'           => 'Одобрить',
    'groups.reject'            => 'Отклонить',
    'groups.members'           => 'Участники',
    'groups.no_members'        => 'Нет одобренных участников.',
    'groups.suspend'           => 'Заблокировать',
    'groups.remove'            => 'Удалить',
    'groups.suspended_members' => 'Заблокированные участники',
    'groups.no_suspended'      => 'Нет заблокированных участников.',
    'groups.reinstate'         => 'Восстановить',
    'groups.back_to_groups'    => 'Назад к группам',

    // Thread subscriptions
    'sub.title'              => 'Подписаться на тему',
    'sub.following_email'    => 'Вы подписаны на эту тему и будете получать уведомления по электронной почте о новых ответах.',
    'sub.bookmarked'         => 'Вы добавили эту тему в закладки (без уведомлений по электронной почте).',
    'sub.not_following'      => 'Вы не подписаны на эту тему.',
    'sub.follow_email'       => 'Подписаться с уведомлениями по e-mail',
    'sub.bookmark'           => 'В закладки (без e-mail)',
    'sub.unfollow'           => 'Отписаться',
    'sub.back_to_thread'     => 'Назад к теме',
    'sub.follow'             => 'Подписаться',
    'sub.confirm_title'      => 'Подтверждение действия',
    'sub.confirm_remove'     => 'Вы уверены, что хотите отписаться от этой темы?',
    'sub.confirm_bookmark'   => 'Заменить подписку на закладку (без уведомлений по электронной почте)?',
    'sub.confirm_yes'        => 'Да, подтвердить',
    'sub.confirm_cancel'     => 'Отмена',
    'sub.forum_title' => 'Подписаться на форум',
    'sub.forum_following_email' => 'Вы сейчас следите за этим форумом и будете получать уведомления по электронной почте о новых темах и ответах.',
    'sub.forum_bookmarked' => 'Вы добавили этот форум в закладки (без уведомлений по email).',
    'sub.forum_not_following' => 'Вы не следите за этим форумом.',
    'sub.back_to_forum' => 'Назад к форуму',
    'sub.confirm_remove_forum' => 'Вы уверены, что хотите отписаться от этого форума?',
    'sub.confirm_bookmark_forum' => 'Изменить подписку на форум на закладку (без уведомлений по email)?',

    // Moderation
    'mod.delete_thread'            => 'Удалить тему',
    'mod.delete_message'           => 'Удалить сообщение',
    'mod.approve_message'          => 'Одобрить сообщение',
    'mod.close_thread'             => 'Закрыть тему',
    'mod.reopen_thread'            => 'Открыть тему',
    'mod.delete_thread_confirm'    => 'Вы уверены, что хотите безвозвратно удалить тему «{subject}» и все ответы к ней? Это действие нельзя отменить.',
    'mod.delete_message_confirm'   => 'Вы уверены, что хотите удалить это сообщение пользователя {author}? Ответы на него будут привязаны к предыдущему сообщению в теме.',
    'mod.approve_confirm'          => 'Одобрить следующее сообщение пользователя {author}, чтобы оно стало видно всем читателям?',
    'mod.close_confirm'            => 'Закрыть тему «{subject}»? После закрытия новые ответы будут запрещены.',
    'mod.open_confirm'             => 'Открыть тему «{subject}», чтобы участники снова могли оставлять ответы?',
    'mod.sticky_thread'            => 'Закрепить тему',
    'mod.unsticky_thread'          => 'Открепить тему',
    'mod.sticky_confirm'           => 'Закрепить тему «{subject}»? Она будет закреплена в верхней части списка форума.',
    'mod.unsticky_confirm'         => 'Открепить тему «{subject}»? Она вернётся к обычному порядку сортировки.',
    'mod.yes_delete'               => 'Да, удалить',
    'mod.approve'                  => 'Одобрить',
    'mod.close'                    => 'Закрыть тему',
    'mod.reopen'                   => 'Открыть тему',
    'mod.sticky'                   => 'Закрепить',
    'mod.unsticky'                 => 'Открепить',
    'mod.cancel'                   => 'Отмена',
    'mod.move_title'               => 'Переместить тему',
    'mod.move_prompt'              => 'Переместить «{subject}» в другой форум:',
    'mod.destination'              => 'Форум назначения',
    'mod.choose_forum'             => '— выберите форум —',
    'mod.move_submit'              => 'Переместить тему',
    'mod.merge_title'               => 'Объединение тем',
    'mod.merge_prompt'              => 'Объединить «{subject}» с другой темой. Сообщения объединяемой темы будут добавлены в конец целевой темы, а подписки этой темы сохранены не будут.',
    'mod.merge_target'              => 'ID целевой темы',
    'mod.merge_target_hint'         => 'Числовой ID темы, в которую нужно выполнить объединение (виден в её URL).',
    'mod.merge_submit'              => 'Объединить темы',
    'mod.merge_error_not_found'     => 'Тема с таким ID не найдена.',
    'mod.merge_error_same_thread'   => 'Выберите другую тему для объединения.',
    'mod.merge_error_failed'        => 'Не удалось объединить с этой темой.',
    'mod.moderate'                 => 'Модерация',
    'mod.queue'                    => 'Очередь на проверку',
    'mod.queue_title'              => 'Очередь сообщений, ожидающих проверки',
    'mod.queue_empty'              => 'Нет сообщений, ожидающих одобрения.',
    'mod.queue_forum'              => 'Форум',
    'mod.queue_posted'             => 'Опубликовано',
    'mod.reports_title'            => 'Жалобы на контент',
    'mod.reports_empty'            => 'Нет открытых жалоб.',
    'mod.reports_message_missing'  => '(сообщение, на которое пожаловались, больше недоступно)',
    'mod.reports_reported'         => 'жалоба подана',
    'mod.reports_reported_by'      => 'от {name}',
    'mod.reports_resolve'          => 'Решить',
    'mod.reports_dismiss'          => 'Отклонить',
    'mod.reports_view'             => 'Просмотреть в теме',
    'mod.users_queue'              => 'Ожидающие пользователи',
    'mod.users_queue_title'        => 'Одобрение ожидающих пользователей',
    'mod.users_queue_empty'        => 'Нет учётных записей, ожидающих одобрения модератора.',
    'mod.users_queue_username'     => 'Имя пользователя',
    'mod.users_queue_email'        => 'Эл. почта',
    'mod.users_queue_ip'           => 'IP-адрес',
    'mod.users_queue_registered'   => 'Зарегистрирован',
    'mod.users_queue_approve'      => 'Одобрить',
    'mod.users_queue_reject'       => 'Отклонить',
    'report.title'                 => 'Пожаловаться на сообщение',
    'report.intro'                 => 'Пожаловаться модераторам на это сообщение пользователя {author}?',
    'report.reason_label'          => 'Причина (необязательно)',
    'report.submit'                => 'Отправить жалобу',

    // Search
    'search.title'           => 'Поиск',
    'search.messages_label'  => 'Поиск сообщений',
    'search.author'          => 'Автор',
    'search.match_type'      => 'Тип совпадения',
    'search.all_words'       => 'Все слова',
    'search.any_word'        => 'Любое слово',
    'search.exact_phrase'    => 'Точная фраза',
    'search.posted_within'   => 'Опубликовано за',
    'search.last_30'         => 'Последние 30 дней',
    'search.last_90'         => 'Последние 90 дней',
    'search.last_year'       => 'Последний год',
    'search.any_time'        => 'За всё время',
    'search.threads_only'    => 'Только первые сообщения в темах',
    'search.forums_label'    => 'Форумы',
    'search.all_forums'      => 'Все форумы',
    'search.submit'          => 'Найти',
    'search.no_results'      => 'Ничего не найдено.',
    'search.showing'         => 'Показано',
    'search.of'              => 'из',
    'search.result'          => 'результат',
    'search.results'         => 'результатов',
    'search.col_subject'     => 'Тема',
    'search.col_author'      => 'Автор',
    'search.col_forum'       => 'Форум',
    'search.col_date'        => 'Дата',

    // -------------------------------------------------------------------------
    // Install
    // -------------------------------------------------------------------------
    'install.page_title'               => 'Установщик Phorum',
    'install.requirements_heading'     => 'Требования',
    'install.requirement_failed'       => 'не пройдено',
    'install.fix_requirements'         => 'Исправьте указанные выше требования, прежде чем продолжить.',
    'install.fix_requirements_hint_1'  => 'Убедитесь, что файлы',
    'install.fix_requirements_hint_and' => 'и',
    'install.fix_requirements_hint_2'  => 'существуют (скопированы из файлов .example) и что учётные данные базы данных верны.',
    'install.errors_heading'           => 'Пожалуйста, исправьте следующее',
    'install.setup_heading'            => 'Настройка сайта и администратора',
    'install.site_name_label'          => 'Название сайта',
    'install.admin_account_heading'    => 'Аккаунт администратора',
    'install.username_label'           => 'Имя пользователя',
    'install.email_label'              => 'Email',
    'install.password_label'           => 'Пароль (мин. 8 символов)',
    'install.confirm_password_label'   => 'Подтверждение пароля',
    'install.submit'                   => 'Установить Phorum',
    'install.complete_page_title'      => 'Установка завершена — Phorum',
    'install.complete_heading'         => 'Установка завершена',
    'install.complete_message'         => 'Схема базы данных создана, и ваш аккаунт администратора готов к использованию.',
    'install.go_to_forum'              => 'Перейти на форум',
    'install.admin_panel'              => 'Панель администратора',
    'install.error_site_name_required'  => 'Необходимо указать название сайта.',
    'install.error_username_required'   => 'Необходимо указать имя пользователя администратора.',
    'install.error_username_format'     => 'Имя пользователя должно содержать от 3 до 50 символов (только буквы, цифры, _ . -).',
    'install.error_email_required'      => 'Необходимо указать корректный адрес электронной почты администратора.',
    'install.error_password_min_length' => 'Пароль администратора должен содержать не менее 8 символов.',
    'install.error_passwords_mismatch'  => 'Пароли не совпадают.',
    'install.error_failed'              => 'Установка не удалась: {message}',

    // -------------------------------------------------------------------------
    // Upgrade (existing Phorum 6 database → Phorum 10)
    // -------------------------------------------------------------------------
    'upgrade.page_title'          => 'Обновление Phorum',
    'upgrade.detected_heading'    => 'Обнаружена существующая база данных Phorum 6',
    'upgrade.detected_message'    => 'Эта база данных была создана Phorum 6. Phorum 10 совместим по схеме с Phorum 6 — существующие данные не будут изменены, удалены или преобразованы.',
    'upgrade.up_to_date'          => 'Изменения схемы не требуются — эта база данных уже актуальна.',
    'upgrade.new_tables_heading'  => 'Будут добавлены следующие новые таблицы:',
    'upgrade.new_patches_heading' => 'Будут применены следующие обновления схемы:',
    'upgrade.submit'              => 'Продолжить',
    'upgrade.complete_page_title' => 'Обновление завершено — Phorum',
    'upgrade.complete_heading'    => 'Обновление завершено',
    'upgrade.complete_message'    => 'Ваша база данных Phorum 6 теперь готова к работе с Phorum 10.',
    'upgrade.go_to_forum'         => 'Перейти на форум',
    'upgrade.admin_panel'         => 'Панель администратора',

    // Error pages
    'error.404_title'        => 'Страница не найдена',
    'error.404_message'      => 'Запрошенная страница не существует.',
    'error.404_return'       => 'Вернуться к списку форумов.',
    'error.403_title'        => 'Доступ запрещён',
    'error.403_message'      => 'У вас нет прав для доступа к этому форуму.',
    'error.403_login'        => 'Войдите',
    'error.403_login_hint'   => 'чтобы получить доступ к форумам, требующим регистрации.',
    'error.403_return'       => 'Вернуться к списку форумов',
    'error.disabled_title'    => 'Сайт недоступен',
    'error.disabled_message'  => 'Этот сайт временно отключён. Пожалуйста, зайдите позже.',
    'error.admin_only_title'   => 'Сайт недоступен',
    'error.admin_only_message' => 'Этот сайт временно закрыт на техническое обслуживание. Пожалуйста, зайдите позже.',
    'error.read_only_title'    => 'Только чтение',
    'error.read_only_message'  => 'Этот сайт сейчас доступен только для чтения. Публикация сообщений и вход в систему временно отключены.',
    'banner.read_only'         => 'Этот сайт сейчас доступен только для чтения — публикация сообщений и вход в систему временно отключены.',

    // -------------------------------------------------------------------------
    // Announcements
    // -------------------------------------------------------------------------
    'announcements.heading' => 'Объявления',

    // -------------------------------------------------------------------------
    // Attachments
    // -------------------------------------------------------------------------
    'attachment.label'      => 'Attachments',
    'attachment.add'        => 'Add files',
    'attachment.existing'   => 'Existing attachments',
    'attachment.remove'     => 'Remove',
    'attachment.hint_count' => 'Up to {n} file(s).',
    'attachment.hint_size'  => 'Max {size} per file.',
    'attachment.error_uploads_disabled' => 'Загрузка файлов в настоящее время отключена.',
    'attachment.lightbox_close' => 'Закрыть',
    'attachment.play_video' => 'Воспроизвести видео',
];
